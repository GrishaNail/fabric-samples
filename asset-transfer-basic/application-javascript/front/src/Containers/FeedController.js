...
class FeedController extends Component {
  ...
  dataHandler(data) {
    if (data.type === "FEED") {
      ...
      const newFeed = [
        <Divider key={this.state.feed.length + 1} />,
        <ListItem key={this.state.feed.length + 2}>
          <h1 className={"error"} key="1">
            ERROR
          </h1>
          <ListItemText
            primary={data.payload}
            key="2"
            secondary="Please check your input"
          />
        </ListItem>,
  ...
  componentDidMount() {
    this.props.socket.on("readAssetResponse", (data) => {
      this.dataHandler(data);
    });
    this.props.socket.on("readAllAssetsResponse", (data) => {
      this.dataHandler(data);
    });
    this.props.socket.on("transferAssetResponse", (data) => {
      this.dataHandler(data);
    });
    this.props.socket.on("createAssetResponse", (data) => {
      this.dataHandler(data);
    });
    ...
    return (
      <Paper classes={{ root: "Page-container" }}>
        <FeedNav
          ...
          value={this.props.showFeed}
        />
        <FeedList data={this.state.feed} />
      </Paper>
    );
  }
}
